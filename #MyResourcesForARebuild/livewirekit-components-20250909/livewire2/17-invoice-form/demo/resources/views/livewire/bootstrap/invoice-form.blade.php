<div>
    <form wire:submit.prevent="saveInvoice">
        @csrf
        <div class="form-group {{ $errors->has('invoice.customer_name') ? 'has-error' : '' }}">
            <label>Customer name</label>
            <input wire:model="invoice.customer_name" type="text" name="invoice.customer_name" class="form-control"
                   value="{{ old('invoice.customer_name') }}" required>
            @if($errors->has('invoice.customer_name'))
                <em class="invalid-feedback">
                    {{ $errors->first('invoice.customer_name') }}
                </em>
            @endif
        </div>
        <div class="form-group {{ $errors->has('invoice.customer_email') ? 'has-error' : '' }}">
            <label>Customer email</label>
            <input wire:model="invoice.customer_email" type="email" name="invoice.customer_email" class="form-control"
                   value="{{ old('invoice.customer_email') }}">
            @if($errors->has('invoice.customer_email'))
                <em class="invalid-feedback">
                ...
                <div class="col-lg-5 ml-auto text-right">
                    <table class="table pull-right">
                        ...
                        <tr>
                            <th>Taxes</th>
                            <td width="125">
                                <input type="number" name="invoice.taxes" class="form-control w-75 d-inline"
                                       min="0" max="100" wire:model="invoice.taxes">
                                %
                                @if($errors->has('invoice.taxes'))
                                    <em class="invalid-feedback">
