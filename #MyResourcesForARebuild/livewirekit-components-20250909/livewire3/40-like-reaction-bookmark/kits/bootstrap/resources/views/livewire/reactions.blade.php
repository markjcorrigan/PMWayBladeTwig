<div class="d-flex w-100">
    ...
    <div wire:click="toggleBookmark" style="width: 33.333333%;" @class(['d-flex justify-content-center align-items-center py-2 ms-1 rounded-lg cursor-pointer text-secondary', 'bg-primary text-body' => $hasBookmarked]) role="button">
    ...
    <div style="width: 33.333333%;" class="d-flex justify-content-center align-items-center ms-1 py-1">
        @foreach(config('markable.allowed_values.reaction') as $reaction)
            <button type="button" @class(['ms-1 p-1 btn', 'btn-outline-light' => ! $hasReacted[$reaction], 'btn-primary rounded-lg' => $hasReacted[$reaction]]) wire:click="toggleReaction('{{ $reaction }}')">
                {{ $reaction }}
            </button>
        @endforeach
    </div>
